...
display()
{
    ...
    model_view *= Translate(0, TORSO_HEIGHT, 0) * RotateZ(theta[Head1]) * RotateY(theta[Head2]);
    head();//头部绘制
...
// GLint angle = Head2;
    ...
    mvstack.push(model_view); //保存躯干变换矩阵
    model_view *= Translate(-0.5 * TORSO_WIDTH, 0.9 * TORSO_HEIGHT, 0) * RotateZ(theta[LeftUpperArm]);
    ...
    mvstack.push(model_view); //保存躯干变换矩阵
    model_view *= Translate(0.5 * TORSO_WIDTH, 0.9 * TORSO_HEIGHT, 0) * RotateZ(theta[RightUpperArm]);
    ...
    mvstack.push(model_view); //保存躯干变换矩阵
    model_view *= Translate(-0.5 * TORSO_WIDTH, 0, 0) * RotateZ(theta[LeftUpperLeg]);
    left_upper_leg();//左上腿绘制
    model_view *= Translate(0, -UPPER_LEG_HEIGHT, 0) * RotateZ(theta[LeftLowerLeg]);
    left_lower_leg();//左下腿绘制
    model_view = mvstack.pop();//恢复躯干变换矩阵

    mvstack.push(model_view); //保存躯干变换矩阵
    model_view *= Translate(0.5 * TORSO_WIDTH, 0, 0) * RotateZ(theta[RightUpperLeg]);
    right_upper_leg();//右上腿绘制
    model_view *= Translate(0, -UPPER_LEG_HEIGHT, 0) * RotateZ(theta[RightLowerLeg]);
